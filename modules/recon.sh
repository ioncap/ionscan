...
mod_passive() {
    ...
    tcpdump -nn -e -r "$cap" 2>/dev/null | grep -oE '([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}' | sort | uniq | while read mac; do
...
# [18] FAST SCAN
mod_fast_scan() {
    read -p "Target IP/CIDR (b=back): " t
...
mod_dns() {
    ...
    local ns=$(dig "$d" NS +short | head -n1)
